...
1. **每次功能更新、UI 更新、bug 修复，都必须同步更新更新日志**，以下两个文件一起改：
   - 根目录 `CHANGELOG.md`
   - `frontend/src/data/changelog.js`（网站「更新日志」页面、首页「最近更新」、后台「版本记录」的数据源）
2. 更新日志必须记录**真实发生的修改**，不编造内容。
3. **时间只写日期**：没有明确确认过具体小时分钟时，只写 `YYYY-MM-DD`，不写小时分钟。除非主人明确给出具体时间（如「这条记录写今天 14:30」），否则只能写日期，或者写「时间待确认」。
4. **不允许编造未来时间**：不得把还没发生的时间写成更新时间。
5. **不允许记录未完成、未上线、未验证的功能**：只是本地改了还没 commit / push 的工作，不得写成已发布的版本记录。
6. **版本号规则**：
   - 大功能或架构变化：提升主版本，例如 `v2.0.0`
   - 新增功能模块：提升功能版本，例如 `v1.5.0`
   - 小功能或功能补充：提升修复版本，例如 `v1.4.1`
   - bug 修复：提升修复版本，例如 `v1.4.2`
7. 每次 Claude Code 修改项目，**最后都要检查是否需要更新 changelog**，需要时同步更新上述两个文件。
